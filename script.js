...
function checkDepartures() {
    ...
    if (hasNonIBTDeparture && timeKey !== lastDepartureCheck) {
        ...
        if (activeDepartedVans.length === 1) {
            ...
            spokenMessage = `Attention please. The van to ${van.location} has now departed at ${currentTime}`;
        } else {
            const locations = activeDepartedVans.map(van => van.location).join(' and ');
            spokenMessage = `Attention please. Multiple vans have now departed at ${currentTime}. Vans to ${locations} have left`;
        }
...
function checkUpcomingDepartures() {
    ...
    if (activeUpcomingDepartures.length > 0) {
        ...
        if (activeUpcomingDepartures.length === 1) {
            ...
            spokenMessage = `Attention please. The van to ${activeUpcomingDepartures[0].location} will be departing in 5 minutes at ${activeUpcomingDepartures[0].time}`;
        } else {
            ...
            spokenMessage = `Attention please. Multiple vans will be departing in 5 minutes. Vans to ${locationList} will depart at ${activeUpcomingDepartures[0].time}`;
        }
